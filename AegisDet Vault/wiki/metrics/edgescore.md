...
## Purpose
Define EdgeScore so every experiment calculates it consistently.

## Project specification
Use mAP50-95_fraction / (latency_ms × model_size_MB) only under identical benchmark conditions. Store raw per-image or per-run values whenever possible; a rounded summary alone is insufficient for audit.
