...
## Purpose
Define Recall so every experiment calculates it consistently.

## Project specification
Report TP/(TP+FN), with special attention to the small and occluded subsets. Store raw per-image or per-run values whenever possible; a rounded summary alone is insufficient for audit.
