...
## Purpose
Define Precision so every experiment calculates it consistently.

## Project specification
Report TP/(TP+FP), globally and per class where sample count is sufficient. Store raw per-image or per-run values whenever possible; a rounded summary alone is insufficient for audit.
